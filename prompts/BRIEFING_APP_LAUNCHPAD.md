# Briefing de construção — App de Cotação em Milhas

> Para entregar a um construtor de aplicativos (Launchpad/Hotmart, Lovable,
> Bolt ou similar). Contém a especificação completa: fluxo, telas, campos,
> identidade visual, integração e o prompt do motor.

---

## ⚠️ Leia isto primeiro — os dois pontos onde este app quebra

**1. A geração demora de 3 a 8 minutos.**
Não são segundos. O modelo faz até 20 buscas na web antes de escrever o relatório.
Uma requisição síncrona comum vai estourar o tempo limite e falhar **em 100% das
vezes**. O app precisa de processamento em segundo plano com consulta de status
(a tela pergunta "já ficou pronto?" a cada poucos segundos), ou de um mecanismo
equivalente.

Se a plataforma não suportar isso, **diga antes de construir** — é melhor mudar a
abordagem do que entregar um app que sempre falha.

**2. A chave da API fica no servidor. Nunca no navegador.**
Se a chave for para o front-end, qualquer pessoa abre o inspetor, copia e gasta o
crédito do dono. Toda chamada ao modelo sai do servidor.

---

## O que o app faz

A pessoa informa uma rota aérea. O app pesquisa preços e programas de milhas na
web, calcula se vale mais a pena pagar em dinheiro ou emitir em milhas, e entrega
um relatório com a resposta.

O produto não é "achar passagem barata" — é responder **quanto valem as milhas de
quem pergunta** e a que nível de viagem essa pessoa já tem acesso.
...
```
[1] Formulário  →  [2] Progresso  →  [3] Relatório
```
...
| --- | --- | --- |
| Origem | texto | aceita "GRU" ou "São Paulo" |
| Destino | texto | aceita "CDG" ou "Paris" |
| Data de ida | data | não permitir data passada |
| Passageiros | número | 1 a 9, começa em 2 |

**Com valor padrão (3):**

| Campo | Tipo | Padrão |
| --- | --- | --- |
| Data de volta | data | vazio = só ida. Nunca antes da ida |
| Cabine | seleção | **Executiva** · Premium Economy · Econômica · Primeira Classe |
| Flexibilidade | seleção | **5 dias** · 3 dias · Datas fixas |

**Bloco opcional, recolhido** — rótulo "Perfil do cliente — opcional":

- Milhas que já tem (texto livre — ex.: "Smiles 180k")
- Pontos de banco ou cartão (texto livre — ex.: "Livelo 90k")
- Forçar custo do milheiro em R$ (texto, vazio = o app pesquisa)

Dentro do bloco, este aviso:

> Em branco, o relatório assume que a pessoa parte do zero e calcula pela via mais
> conservadora. Depois mostra quanto cairia com pontos de banco.

Botão: **Gerar cotação**.

### Tela 2 — Progresso

Aparece no lugar do formulário. Mostra a fase atual, sem porcentagem falsa:

- "Pesquisando — mapeando companhias e rotas"
- "Pesquisando — N buscas realizadas"
- "Montando o relatório"

E o aviso: *"Leva de 3 a 8 minutos. Pode deixar esta aba em segundo plano."*

O botão fica desabilitado enquanto processa.

### Tela 3 — Relatório

O texto do relatório, renderizado de forma legível (não em bloco de código).
Abaixo dele:

- Botão **Baixar em PDF**
- Rodapé discreto com a data da pesquisa

---

## Identidade visual

Marca: **Tania Pavão · Milhas e Destinos** — Instagram @passosdopavaomilhas.
Posicionamento premium. Não é site de promoção.

```
Fundo escuro      #080d1a
Azul-tinta        #05080f
Navy              #0b1628
Dourado           #c9a84c    (destaques, botões, rótulos)
Dourado claro     #f5e0a0    (títulos)
Creme             #f7f2ea    (texto)
Cinza-azulado     #8a9bb0    (texto secundário)
```

- **Títulos:** Cormorant Garamond (serifada, leve, com itálico nos destaques)
- **Texto e formulário:** Inter
- **Cantos retos.** Sem borda arredondada, sem sombra colorida, sem gradiente chamativo
- **Botão principal:** fundo dourado, texto escuro, maiúsculas, espaçamento entre letras
- **Campos:** fundo quase preto, borda dourada fina, sem arredondamento

Tom das mensagens: direto e adulto. Sem "Oba!", sem emoji de comemoração, sem
exclamação. Erro diz o que houve e o que fazer.

---

## Integração com o modelo

- **Modelo:** Claude (Anthropic) ou GPT (OpenAI) — o que a plataforma suportar
- **Busca na web: obrigatória.** Sem ela o modelo inventa preço. Se não houver
  como ativar, o app não funciona — avise antes de construir
- **Prompt de sistema:** o bloco inteiro da última seção deste briefing
- **Mensagem do usuário:** a data de hoje, seguida dos campos do formulário no
  formato do bloco "Preencha e envie" que está no fim do prompt
- **Tamanho da resposta:** permitir resposta longa (o relatório passa de 2.000
  palavras). Se houver limite configurável, use o máximo

### Verificação antes de mostrar o relatório

Se o texto gerado não contiver as seis seções numeradas (`1. O VEREDITO` até
`6. O QUE FAZER AGORA`), mostre um aviso acima do relatório:

> Este relatório saiu incompleto. Confira antes de usar.

### Custo

Cada geração custa cerca de US$ 1 a US$ 2,50 em API, dependendo do modelo.
Deixe isso visível para o dono do app, e configure um teto de gasto.

---

## O que NÃO fazer

- Não colocar a chave da API no navegador
- Não fazer requisição síncrona esperando resposta imediata
- Não inventar barra de progresso com porcentagem — não há como saber
- Não deixar o app público sem senha: cada geração custa dinheiro real do dono,
  e uma URL vazada vira prejuízo
- Não resumir nem reescrever o relatório do modelo. Exibir como veio
- Não traduzir nem alterar o prompt de sistema abaixo

---

## Prompt de sistema — copiar integralmente, sem alterar

O bloco abaixo, do começo ao fim, é o prompt de sistema do app.

**Uma exceção:** a seção **"A entrega em PDF"** foi escrita para uso em conversa
de chat, onde o modelo oferece o arquivo. **Dentro do app isso é tarefa do app**,
pelo botão "Baixar em PDF" da tela 3. Remova essa seção do prompt de sistema, ou
substitua por uma linha: *"Não ofereça PDF — o aplicativo cuida disso."*

Se ela ficar, o modelo vai encerrar o relatório perguntando se a pessoa quer um
PDF que ele não tem como gerar, e a pessoa fica esperando.
